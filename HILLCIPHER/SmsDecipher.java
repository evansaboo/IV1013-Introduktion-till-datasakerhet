...
public class SmsDecipher {
	...
	public static void main(String[] args){
		...
		if(args.length < 2 || args.length > 2){
			errorMsg("usage: <invkeyfile> <cipher-textfile>");
		}
		...
		try{
			...
			while((line = key_bufferedReader.readLine()) != null){
				...
				for(String v: line.split(" ")){
					if(j > 2 || y > 2){
						errorMsg("Inverse key matrix does not have the dimensions 3x3. Please provide a 3x3 matrix.");
					}
					try{
						dMatrix[y][j] = Integer.parseInt(v);
					} catch(NumberFormatException e){
						errorMsg("The character or word '"+v+"' is not a number.");
					}
					if(dMatrix[y][j] < 0 || dMatrix[y][j] > 25){ errorMsg("Value '"+v+"' is out of bounds: All values in the matrix must be in range of 0 to 25.");}
					j++;
				}
				if(j < 3){
					errorMsg("Inverse key matrix does not have the dimensions 3x3. Please provide a 3x3 matrix.");
